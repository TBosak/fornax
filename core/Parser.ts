...
export class Parser extends parse5.Parser<Htmlparser2TreeAdapterMap> {
  public patches: LRUCache<string, (domElement: Element, done?: Function) => void>;

  constructor(cacheCapacity: number = 100) {
    super();
    this.patches = new LRUCache<string, (domElement: Element, done?: Function) => void>(cacheCapacity);
  }
  ...
  createPatch(source: string): (domElement: Node, done?: Function) => void {
    ...
    const traverse = (node: any, parentId: string = 'root', index: number = 0) => {
      ...
      if (node.nodeName && node.nodeName !== '#text' && node.nodeName !== '#document-fragment') {
        ...
        if (hasChildren) node.childNodes.forEach((child: any, idx: number) => traverse(child, id, idx));
